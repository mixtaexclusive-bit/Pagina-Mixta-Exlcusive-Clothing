...
const rowToProduct = (row, index) => {
  ...
  const sizes = parseList(row.tallas || row.talla || row.sizes || row.size);
  ...
  return {
    ...
    sortOrder: parseOrder(row.orden || row.order || row.prioridad || row.priority)
  };
};

const stripProductMeta = ({ sheetRow, addedAt, sortOrder, ...product }) => product;
